...
class Sprite : public Rendering::RenderObject {
public:
    Sprite();
    ...
    inline void Move(float offsetX, float offsetY) {
        m_transform.SetPosition(m_transform.GetPosition().x + offsetX,
                                m_transform.GetPosition().y + offsetY);
    }
    inline void Move(const Vector2f& offset) {
    ...
    inline Rectf Bounds() const {
        return {{m_transform.GetPosition()}, {m_transform.GetPosition() + m_scaledTextureSize}};
    }
